...
export const getLeagueById = async (id: string): Promise<League> => {
    return await mainClient.get(`leagues/${id}`);
};

export const saveLeague = async (
    data: PersistLeagueRequest,
): Promise<string> => {
    return await mainClient.post("leagues", data);
};
...
export const getFixture = async (
    ...
    return await mainClient.get<LeagueFixtureMatchResponse[]>(
...
export const getStandings = async (
    ...
    return await mainClient.get<ScoreBoardResponse[]>(
        `leagues/${leagueId}/standings`,
    );
};

export const assignCoordinator = async (leagueId: string, data: { userId: string }): Promise<Boolean> => {
    return await mainClient.post<Boolean>(`leagues/${leagueId}/coordinator`, null, {params: data})
}

export const updateMatchDate = async (leagueId: string, matchId: string, data: {
    'match-date': Date | null
}): Promise<Boolean> => {
    return await mainClient.put(`leagues/${leagueId}/match/${matchId}/update-date`, null, {
        params: data,
    });
};

export const approveMatchResult = async (leagueId: string, matchId: string): Promise<MatchScore> => {
    return await mainClient.put<MatchScore>(`leagues/${leagueId}/match/${matchId}/approve`);
};

export const updateLeagueMatchScore = async (leagueId: string, matchId: string, data: MatchScore): Promise<MatchScoreResponse> => {
    return await mainClient.put<MatchScoreResponse>(`leagues/${leagueId}/match/${matchId}/update-score`, data);
};
