...
func (r *PlayerRepository) GetById(ctx context.Context, id int64) (*player.Player, error) {
	...
	query := `SELECT id, uuid, name, surname, sex,user_id FROM players WHERE id=$1`
	err := r.db.QueryRowContext(ctx, query, id).
		Scan(&player.ID, &player.Uuid, &player.Name, &player.Surname, &player.Sex, &player.UserId)
	if err != nil {
...
func (r *PlayerRepository) GetByUuid(ctx context.Context, uuid string) (*player.Player, error) {
	player := &player.Player{}
	query := `SELECT id, uuid, name, surname,sex, user_id FROM players WHERE uuid=$1`
...
func (r *PlayerRepository) List(ctx context.Context, name string) ([]*player.Player, error) {
	...
	query := `SELECT id, uuid, name, surname,sex, user_id FROM players WHERE name ILIKE '%' || $1 || '%'`
	...
	for rows.Next() {
		player := &player.Player{}
		err := rows.Scan(&player.ID, &player.Uuid, &player.Name, &player.Surname, &player.Sex, &player.UserId)
...
func (r *PlayerRepository) Save(ctx context.Context, persistPlayer *player.PersistPlayer) (int64, error) {
	query := `INSERT INTO players (uuid, name, surname,sex, user_id) VALUES (gen_random_uuid(), $1, $2, $3,$4) RETURNING id`
	var id int64
