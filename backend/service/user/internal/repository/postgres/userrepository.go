...
func (r *UserRepository) FindById(ctx context.Context, userId string) (*user.UserData, error) {
	...
	err = sqlscan.Get(ctx, r.GetExecutor(ctx), &entity, query, args...)
	if err != nil {
		log.Printf("DB Hatası! SQL: %s | Args: %v | Hata: %v", query, args, err)
		return nil, fmt.Errorf("veritabanı hatası: %v", err)
	}

	// sqlscan hata dönmediyse, entity nesnesi veritabanı verileriyle dolmuştur.
	// Şimdi DB entity'sini, domain katmanının beklediği user.User struct'ına eşliyoruz:
